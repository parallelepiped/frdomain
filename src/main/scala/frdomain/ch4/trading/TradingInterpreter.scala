...
trait TradingInterpreter extends Trading[Account, Trade, ClientOrder, Order, Execution, Market] {
  ...
  def execute(market: Market, brokerAccount: Account): ReaderT[List, Order, Execution] = kleisli[List, Order, Execution] { order =>
    order.items.map { item =>
      Execution(brokerAccount, item.ins, "e-123", market, item.price, item.qty)
    }
  }

  def allocate(accounts: List[Account]): ReaderT[List, Execution, Trade] = kleisli[List, Execution, Trade] { execution =>
