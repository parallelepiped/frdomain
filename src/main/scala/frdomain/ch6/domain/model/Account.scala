...
object Account {
  ...
  private def validateAccountAlreadyClosed(a: Account): ValidationNel[String, Account] = {
    if (a.dateOfClose isDefined) s"Account ${a.no} is already closed".failureNel[Account]
    else a.successNel[String]
  }

  private def validateCloseDate(a: Account, cd: Date): ValidationNel[String, Date] = {
  ...
  private def checkBalance(a: Account, amount: Amount): ValidationNel[String, Account] = {
